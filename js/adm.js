...
import { buscarEventosDoMesAtual, cancelarEventoID } from './sdk_firebase.js'
...
function criarCardEvento (e) {
  ...
  card.innerHTML = `${e.grupo} – ${e.titulo}`

  const lixeira = document.createElement('input')
  lixeira.type = 'button'
  // lixeira.value = 'X'
  // lixeira.className = 'btn btn-danger'
  lixeira.onclick = () => {cancelarEventoID(e.id); generateCalendario()}
